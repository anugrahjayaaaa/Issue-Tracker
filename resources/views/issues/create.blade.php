            ...
            <div class="row g-2 mb-3">
                <div class="col-md-3">
                    ...
                    <select name="type" class="form-select">
                        @foreach ($project->issueTypes as $t)<option value="{{ $t->key }}" {{ old('type') == $t->key ? 'selected' : '' }}>{{ $t->name }}</option>@endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    ...
                    <select name="status" class="form-select">
                        <option value="">-</option>
                        @foreach ($project->statuses as $s)<option value="{{ $s->key }}" {{ old('status') == $s->key ? 'selected' : '' }}>{{ $s->name }}</option>@endforeach
                    </select>
                </div>
                <div class="col-md-3">
